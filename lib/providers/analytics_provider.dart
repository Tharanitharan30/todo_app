...
final productivityOverviewProvider =
    Provider<AsyncValue<ProductivityOverview>>((ref) {
  ...
  return tasksAsync.whenData((tasks) {
    final periodTasks = tasks.where((t) {
      final createdInPeriod = t.createdAt
              .isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
          t.createdAt.isBefore(range.end.add(const Duration(milliseconds: 1)));
      final completedInPeriod = t.completedAt != null &&
          t.completedAt!
              .isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
          t.completedAt!.isBefore(range.end.add(const Duration(milliseconds: 1)));
    ...
    final completionRate =
        createdCount <= 0 ? 0.0 : (completedCount / createdCount) * 100;
  ...
  return tasksAsync.whenData((tasks) {
    ...
    if (period == AnalyticsPeriod.today) {
      ...
      for (int hour = 0; hour < 24; hour += 4) {
        final slotStart = DateTime(range.start.year, range.start.month, range.start.day, hour);
        final slotEnd = DateTime(range.start.year, range.start.month, range.start.day, hour + 3, 59, 59);

        final count = tasks
            .where((t) =>
                t.status == 'completed' &&
                t.completedAt != null &&
                t.completedAt!.isAfter(slotStart.subtract(const Duration(milliseconds: 1))) &&
                t.completedAt!.isBefore(slotEnd.add(const Duration(milliseconds: 1))))
            .length;

        final label = '${hour.toString().padLeft(2, '0')}:00';
        points.add(TaskCompletionPoint(label: label, count: count, date: slotStart));
      }
      ...
      for (int i = 6; i >= 0; i--) {
        ...
        final count = tasks
            .where((t) =>
                t.status == 'completed' &&
                t.completedAt != null &&
                t.completedAt!.isAfter(dayStart.subtract(const Duration(milliseconds: 1))) &&
                t.completedAt!.isBefore(dayEnd.add(const Duration(milliseconds: 1))))
            .length;

        final label = weekdayNames[d.weekday - 1];
        points.add(TaskCompletionPoint(label: label, count: count, date: d));
      }
    } else if (period == AnalyticsPeriod.thirtyDays) {
      for (int i = 29; i >= 0; i -= 5) {
        final d = DateTime.now().subtract(Duration(days: i));
        final dayStart = DateTime(d.year, d.month, d.day);
        final dayEnd = dayStart.add(const Duration(days: 4, hours: 23, minutes: 59, seconds: 59));
      ...
      for (int i = 11; i >= 0; i--) {
        ...
        final count = tasks
            .where((t) =>
                t.status == 'completed' &&
                t.completedAt != null &&
                t.completedAt!.isAfter(mStart.subtract(const Duration(milliseconds: 1))) &&
                t.completedAt!.isBefore(mEnd.add(const Duration(milliseconds: 1))))
            .length;

        final label = monthNames[mDate.month - 1];
        points.add(TaskCompletionPoint(label: label, count: count, date: mDate));
      }
    }
...
final tasksCreatedVsCompletedProvider =
  ...
  return tasksAsync.whenData((tasks) {
    ...
    if (period == AnalyticsPeriod.today || period == AnalyticsPeriod.sevenDays) {
      for (int i = 6; i >= 0; i--) {
        ...
        final created = tasks
            .where((t) =>
                t.createdAt.isAfter(dayStart.subtract(const Duration(milliseconds: 1))) &&
                t.createdAt.isBefore(dayEnd.add(const Duration(milliseconds: 1))))
            .length;
        ...
        final label = weekdayNames[d.weekday - 1];
        points.add(CreatedVsCompletedPoint(label: label, created: created, completed: completed));
      }
    } else if (period == AnalyticsPeriod.thirtyDays) {
      for (int i = 3; i >= 0; i--) {
        ...
        final created = tasks
            .where((t) =>
                t.createdAt.isAfter(wStart.subtract(const Duration(days: 1))) &&
        ...
        final completed = tasks
            .where((t) =>
                t.status == 'completed' &&
                t.completedAt != null &&
                t.completedAt!.isAfter(wStart.subtract(const Duration(days: 1))) &&
                t.completedAt!.isBefore(wEnd.add(const Duration(days: 1))))
            .length;
      ...
      for (int i = 5; i >= 0; i--) {
        ...
        final created = tasks
            .where((t) =>
                t.createdAt.isAfter(mStart.subtract(const Duration(milliseconds: 1))) &&
                t.createdAt.isBefore(mEnd.add(const Duration(milliseconds: 1))))
            .length;
...
final categoryProductivityProvider =
  ...
  return tasksAsync.whenData((tasks) {
    final completedTasks = tasks.where((t) {
      ...
      return d.isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
    ...
    countMap.forEach((cat, count) {
      final pct = totalCompleted <= 0 ? 0.0 : (count / totalCompleted) * 100;
      list.add(CategoryProductivityInfo(
        category: cat,
...
final priorityAnalysisProvider =
  ...
  return tasksAsync.whenData((tasks) {
    ...
    for (final prio in priorities) {
      final prioTasks =
          tasks.where((t) => t.priority.toLowerCase() == prio).toList();
      final total = prioTasks.length;
      final completed = prioTasks.where((t) => t.status == 'completed').length;
...
final taskStatusDistributionProvider =
  ...
  return tasksAsync.whenData((tasks) {
    ...
    final overdue = tasks
        ...
        .length;
    final pending =
        tasks.where((t) => t.status != 'completed' && !_isOverdueTask(t, now)).length;
...
final financeAnalyticsOverviewProvider =
  ...
  final totalInc = incomes
      .where((i) =>
          i.date.isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
          i.date.isBefore(range.end.add(const Duration(milliseconds: 1))))
      .fold(0.0, (sum, i) => sum + i.amount);

  final totalExp = expenses
      .where((e) =>
          e.date.isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
          e.date.isBefore(range.end.add(const Duration(milliseconds: 1))))
      .fold(0.0, (sum, e) => sum + e.amount);

  final balance = totalInc - totalExp;
  final rate = totalInc <= 0 ? 0.0 : ((totalInc - totalExp) / totalInc) * 100;
...
final expenseCategoryAnalysisProvider =
  ...
  return expensesAsync.whenData((expenses) {
    ...
    map.forEach((cat, amt) {
      ...
      list.add(ExpenseCategoryData(category: cat, amount: amt, percentage: pct));
    });
...
final incomeVsExpenseMonthlyTrendProvider =
  ...
  for (int i = 5; i >= 0; i--) {
    ...
    final inc = incomes
        .where((i) =>
            i.date.isAfter(mStart.subtract(const Duration(milliseconds: 1))) &&
            i.date.isBefore(mEnd.add(const Duration(milliseconds: 1))))
        .fold(0.0, (sum, i) => sum + i.amount);

    final label = monthNames[mDate.month - 1];
    list.add(IncomeVsExpensePoint(monthLabel: label, income: inc, expense: exp));
  }
...
final savingsTrendProvider =
    Provider<AsyncValue<List<SavingsTrendPoint>>>((ref) {
...
final savingsRateTrendProvider =
  ...
  return comparisonAsync.whenData((list) {
    return list.map((item) {
      final rate = item.income <= 0 ? 0.0 : ((item.income - item.expense) / item.income) * 100;
      return SavingsRateTrendPoint(monthLabel: item.monthLabel, rate: rate < 0 ? 0 : rate);
    }).toList();
  });
});

final budgetAnalyticsProvider =
    Provider<AsyncValue<BudgetAnalyticsSummary>>((ref) {
...
final subscriptionAnalyticsProvider =
  ...
  final summary = summaryAsync.value ?? const SubscriptionSummary(monthlyTotal: 0, yearlyTotal: 0, activeCount: 0);

  Map<String, int> countMap = {'Weekly': 0, 'Monthly': 0, 'Quarterly': 0, 'Yearly': 0};

  for (final sub in subs) {
    if (!sub.active) continue;
    final cycle = sub.billingCycle[0].toUpperCase() + sub.billingCycle.substring(1).toLowerCase();
...
final productivityHighlightProvider =
  ...
  final completedTasks = tasks.where((t) =>
      t.status == 'completed' &&
      t.completedAt != null &&
      t.completedAt!.isAfter(range.start.subtract(const Duration(milliseconds: 1))) &&
      t.completedAt!.isBefore(range.end.add(const Duration(milliseconds: 1))));
...
final productivityInsightsProvider =
    Provider<AsyncValue<List<InsightItem>>>((ref) {
  ...
  if (overviewAsync.isLoading || highlightsAsync.isLoading || financeOverviewAsync.isLoading) {
    return const AsyncValue.loading();
  }

  final overview = overviewAsync.value ?? const ProductivityOverview(tasksCreated: 0, tasksCompleted: 0, completionRate: 0, overdueCount: 0, avgCompletedPerDay: 0, importantCount: 0);
  final highlights = highlightsAsync.value ?? const ProductivityHighlightData(highestSpendingCategory: 'None', highestSpendingAmount: 0, highestSpendingPercentage: 0, mostProductiveCategory: 'None', mostProductiveCategoryCount: 0, bestProductivityDay: 'None', bestProductivityDayCount: 0, avgDailySpending: 0);
  final finance = financeOverviewAsync.value ?? const FinanceOverviewData(income: 0, expenses: 0, balance: 0, savingsRate: 0);
  ...
  if (overview.completionRate >= 70 && overview.tasksCompleted > 0) {
    insights.add(
      InsightItem(
        ...
        message: 'You completed ${overview.completionRate.toStringAsFixed(0)}% of your tasks during this period!',
    ...
    insights.add(
      InsightItem(
        ...
        message: 'You have ${overview.overdueCount} overdue tasks. Consider reviewing your upcoming deadlines.',
  ...
  if (highlights.mostProductiveCategory != 'None' && highlights.mostProductiveCategoryCount > 0) {
    insights.add(
      InsightItem(
        title: '${highlights.mostProductiveCategory} is your most active category',
        message: 'You completed ${highlights.mostProductiveCategoryCount} tasks in ${highlights.mostProductiveCategory}.',
  ...
  // Financial Insights
  if (highlights.highestSpendingCategory != 'None' && highlights.highestSpendingAmount > 0) {
    insights.add(
      InsightItem(
        title: '${highlights.highestSpendingCategory} is your highest spending category',
        message: 'You spent ₹${highlights.highestSpendingAmount.toStringAsFixed(0)} (${highlights.highestSpendingPercentage.toStringAsFixed(0)}% of total expenses).',
  ...
  if (finance.savingsRate >= 40 && finance.income > 0) {
    insights.add(
      InsightItem(
        ...
        message: 'Your savings rate reached ${finance.savingsRate.toStringAsFixed(0)}% with a net balance of ₹${finance.balance.toStringAsFixed(0)}.',
  ...
  if (highlights.bestProductivityDay != 'None' && highlights.bestProductivityDayCount > 0) {
    insights.add(
      InsightItem(
        title: '${highlights.bestProductivityDay} is your peak productivity day',
        message: 'You completed the highest number of tasks (${highlights.bestProductivityDayCount}) on ${highlights.bestProductivityDay}s.',
